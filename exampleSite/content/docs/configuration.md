...
| `navLinks` | array of tables | Extra top-bar links. A `name` + `url` is a plain link (external URLs open in a new tab). A `name` plus nested `columns` joins the mega-menu: one shared panel slides and resizes between triggers as you hover. Each column has a `title` and `links` (each `name`, `url`, optional `icon` Lucide name, and `desc`). |
